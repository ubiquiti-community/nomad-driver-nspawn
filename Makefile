...
changelogfmt: ## Format changelog GitHub links
	...
	@sed -E 's|([^\[])\[GH-([0-9]+)\]|\1[[GH-\2](https://github.com/hashicorp/nomad-driver-nspawn/issues/\2)]|g' CHANGELOG.md > changelog.tmp && mv changelog.tmp CHANGELOG.md
...
clean: ## Cleanup previous build
	...
	rm -f ./build/nomad-driver-nspawn

.PHONY: dev
dev: clean build/nomad-driver-nspawn ## Build the nomad-driver-nspawn plugin
...
dist/%.zip: dist/%/nomad-driver-nspawn
